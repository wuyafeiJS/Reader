...
export async function getFirstRenderChapter (ctx) {
  let bookshelf, chapter, nextChapter, chapters, currentChapter
  ...
  try {
    ...
    if (bookshelf.novel.countChapter != num + 1 && !nextChapter.content) {
      nextChapter = chapter.chapters[num + 1]
      if (!nextChapter.content) {
        ...
        await Chapter.update({_id: bookshelf.chapter}, { chapters: chapter.chapters })
        // await chapter.save()
      }
    }
  } catch (e) {
  ...
  currentChapter = chapter.chapters[num]
  if (!currentChapter.content) {
    const url = `${currentChapter.postfix}`
    try {
      const content = await Crawler.getChapterContent(url)
