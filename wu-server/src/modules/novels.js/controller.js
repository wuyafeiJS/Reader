...
export async function searchFromZH (ctx) {
  ...
  // 起点api
  const qdUrl = `https://www.qidian.com/ajax/Search/AutoComplete?_csrfToken=&siteid=1&query=${word}`
  try {
    var body = await Crawler.getBody(qdUrl)
  } catch (e) {
    Handle.sendEmail(e.message)
    ctx.throw(422, err.message)
  }
...
export async function searchFromBQK (ctx) {
  ...
  // 书趣阁搜索网站
  const url = `http://zhannei.baidu.com/cse/search?s=920895234054625192&q=${name}`
  try {
  ...
  for (let i = 0; i < length; i++) {
    const title = $('.result-game-item-title a')[i].attribs.title
    const img = $('.result-game-item-pic img')[i].attribs.src
    const url = $('.result-game-item-title a')[i].attribs.href
    const introduction = $('.result-game-item-desc').eq(i).text().replace(/\s/g, "")
    const author = $('.result-game-item-info').eq(i).find('p:first-child span:last-child').text().replace(/\s/g, "")
    const type = $('.result-game-item-info').eq(i).find('p:nth-child(2) span:last-child').text().replace(/\s/g, "")
    let json = {
      title: title,
      img: img,
      url: url,
...
export async function getNovel (ctx) {
  ...
  if (novel) {
    ...
    const updateTime = $('#info p').eq(2).text()
    const img = $('#fmimg img')[0].attribs.src
    novelInfo.url = url
    novelInfo.name = $('#info h1').text()
    novelInfo.author = author.split('：')[1].trim()
    novelInfo.img = config.crawlerUrl + img
    novelInfo.updateTime = updateTime.substring(5, updateTime.length)
    novelInfo.introduction = $('#intro').text()
    novel = new Novel(novelInfo)
    try {
      await novel.save()
    } catch (e) {
      Handle.sendEmail(e.message)
      ctx.throw(423, e.message)
    }
    const novelId = novel.id
    await Crawler.getNovel($, novelId, url)
    try {
      var titles = await Chapter.getTitles(novelId)
      var chapters = titles[0].chapters
      var count = chapters.length
    } catch (e) {
      Handle.sendEmail(e.message)
      ctx.throw(424, e.message)
    }
    novel.lastChapterTitle = chapters[chapters.length - 1].title
    novel.countChapter = count
    try {
      await novel.save()
    } catch (e) {
      Handle.sendEmail(e.message)
      ctx.throw(425, e.message)
    }
    const response = novel.toJSON()
    response.join = false
    ctx.body = {
